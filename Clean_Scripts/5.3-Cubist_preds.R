...
load(paste0(HomeDir,"Clean_Output/4.Cubist/4.3-Cubist_15_30.RData"))

### Copy two stacks, one for granulo, the other for coarse elements
#covariates_EG <- covariates
covariates_granulo <- covariates

# ### check if I need to eliminate or not any variable (clc) from the coarse.Cub.15_30 model
# 
# ### clc06
# setdiff(unique(covariates_granulo[["clc06"]]) , unlist(unique(levels(granulo.data_15_30$clc06))) )
# ### [1] 123 212 244 335 412 423 521 522 523                                              ### 9 classes are missing
# setdiff(unique(covariates_EG[["clc06"]]) , unlist(unique(levels(EG.data_15_30$clc06))) )
# ### [1] 111 123 212 213 244 335 421 423 521 522 523                                  ### 11 classes for Eg.granulo
# ### [1] 111 123 212 213 244 335 421 423 521 522 523

# ### bdforet  
# unique(levels(granulo.data_15_30$bdforet))        # [1] "0"  "1"  "2"  "3"  "4"  "5"  "6"  "30" "40" "50" All classes present
# ### and for coarse elements
# unique(levels(EG.data_15_30$bdforet)) 
# ### [1] "0"  "1"  "2"  "3"  "4"  "5"  "6"  "30" "40" "50"
#
# setdiff(unique(covariates_granulo[["ecoclim"]]) , unlist(unique(levels(granulo.data_15_30$ecoclim))) )
# setdiff(unique(covariates_EG[["ecoclim"]]) , unlist(unique(levels(EG.data_15_30$ecoclim))) )
### Level 3, as for 0-5 cm

# ### For mat11 
# setdiff(unique(covariates_granulo[["mat11"]]) , unlist(unique(levels(granulo.data_15_30$mat11))) )### All classes present :)
# setdiff(unique(covariates_EG[["mat11"]]) , unlist(unique(levels(EG.data_15_30$mat11))) )### All classes present :)
# 
# ### For modelclc
# setdiff(unique(covariates_granulo[["modelclc"]]) , unlist(unique(levels(granulo.data_15_30$modelclc))) )### all present in the training dataset
# setdiff(unique(covariates_EG[["modelclc"]]) , unlist(unique(levels(EG.data_15_30$modelclc))) )### all present in the training dataset
# 
# ### For soil1
# setdiff(unique(covariates_granulo[["soil1"]]) , unlist(unique(levels(granulo.data_15_30$soil1))) )### class 25 (glacier) not present in training dataset
# setdiff(unique(covariates_EG[["soil1"]]) , unlist(unique(levels(EG.data_15_30$soil1))) )### class 25 (glacier) not present in training dataset


### I went back to the script 4.3-Cubist_15_30 and eliminated clc06 from the predictor variables, to ncrease the predicted surface
...
beginCluster(7)
system.time(silt.alr.cub.15_30 <- clusterR(covariates_granulo, predict, args = list(limon.Cub.15_30, committees = 20),
                                         filename = "silt.alr.cub.15_30.tif", format = "GTiff",
                                         factors=factor_list.granulo, na.rm=T, inf.rm=T, progress = "text", overwrite = T))
endCluster()


beginCluster(7)
system.time(argile.alr.cub.15_30 <- clusterR(covariates_granulo, predict, args = list(argile.Cub.15_30, committees = 20),
                                           filename = "argile.alr.cub.15_30.tif", format = "GTiff",
                                           factors=factor_list.granulo, na.rm=T,inf.rm=T, progress = "text", overwrite = T))
endCluster()


par(mfrow=c(1,3))
plot(silt.alr.cub.15_30)
plot(argile.alr.cub.15_30)
