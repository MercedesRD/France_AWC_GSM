...
granulo.100_200.s <- stack(argile.alr.cub.100_200, silt.alr.cub.100_200,
                        ...
                        argile.preds.RK.100_200,limon.preds.RK.100_200,sable.preds.RK.100_200)
...
awc.100_200.s <- stack(SMFC_100_200, SMPWP_100_200,ru_vol_100_200,ru_mm_100_200,coarse.qrF.mean.100_200,awc_mm_100_200)
...
dir.create(paste0(HomeDir,"Clean_Output/7.6-AWC_Taylor_100_200"))
...
tic <- Sys.time()
argile.alr.v <- getValues(argile.alr.preds.100_200)
silt.alr.v <- getValues(limon.alr.preds.100_200)

### calculate correlation
cor.clay.limon.alr.100_200 <- cor.test(x=argile.alr.v, y=silt.alr.v, na.rm=TRUE, method="pearson")
cor.clay.limon.alr.100_200$estimate
tac <- Sys.time()
tac-tic
rm(tac,tic)
...
t_layer <- 1000 ### thickness of layer in mm
...
## my intput raster stack is granulo.100_200.s
names(granulo.100_200.s)
...
#  s[[1]] "argile.alr.cub.100_200" 
#  s[[2]] "silt.alr.cub.100_200" 
#  s[[3]] "argile.alr.ckR.100_200" 
#  s[[4]] "limon.alr.ckR.100_200"  
#  s[[5]] "argile.alr.ckSTD.100_200"
#  s[[6]] "limon.alr.ckSTD.100_200"
#  s[[7]] "argile.alr.preds.100_200"
#  s[[8]] "limon.alr.preds.100_200" 
#  s[[9]] "clay" 
#  s[[10]] "silt" 
#  s[[11]] "sand"
...
rho <- cor.clay.limon.alr.100_200$estimate

beginCluster(2)
var_w20_input_100_200 <- clusterR(granulo.100_200.s, ff, export = list("Uncertainty_input","ptf", "rho"),
                          filename = paste0("varInput_w20_",depth,".tif"), format = "GTiff",
                          na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w20_input_100_200 <- raster(paste0("varInput_w20_",depth,".tif"))
...
beginCluster(2)
var_w42_input_100_200  <- clusterR(granulo.100_200.s, ff, export = list("Uncertainty_input","ptf","rho"),
                          filename = paste0("varInput_w42_",depth,".tif"), format = "GTiff",
                          na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w42_input_100_200 <- raster(paste0("varInput_w42_",depth,".tif"))

par(mfrow=c(3,2))
plot(var_w20_input_100_200)
plot(var_w42_input_100_200)
...
beginCluster(2)
var_w20_ptf_100_200 <- clusterR(granulo.100_200.s, ff, export = list("Uncertainty_PTF","coeff.var.covar"),
                        filename = paste0("varPTF_w20_",depth,".tif"), format = "GTiff",
                        na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w20_ptf_100_200 <- raster(paste0("varPTF_w20_",depth,".tif"))
...
beginCluster(2)
var_w42_ptf_100_200 <- clusterR(granulo.100_200.s, ff, export = list("Uncertainty_PTF","coeff.var.covar"),
                        filename = paste0("varPTF_w42_",depth,".tif"), format = "GTiff",
                        na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

gc()
var_w42_ptf_100_200 <- raster(paste0("varPTF_w42_",depth,".tif"))

plot(var_w20_ptf_100_200)
plot(var_w42_ptf_100_200)
...
var.awc.s <- stack (var_w20_input_100_200, var_w42_input_100_200, var_w20_ptf_100_200, var_w42_ptf_100_200)
...
beginCluster(2)
total_var_w20.100_200 <- clusterR(var.awc.s, calc, args = list(sum_unc_w20),
                                 filename =paste0("Total_var_w20.",depth,".tif"), format = "GTiff",
                                 na.rm=T,inf.rm=T, progress = "text", overwrite = T)
endCluster()

total_var_w20.100_200 <- raster(paste0("Total_var_w20.",depth,".tif"))


beginCluster(2)
total_var_w42.100_200 <- clusterR(var.awc.s, calc, args = list(sum_unc_w42),
                              filename =paste0("Total_var_w42.",depth,".tif"), format = "GTiff",
                              na.rm=T,inf.rm=T, progress = "text", overwrite = T)
endCluster()

total_var_w42.100_200 <- raster(paste0("Total_var_w42.",depth,".tif"))

plot(total_var_w20.100_200)
plot(total_var_w42.100_200)

# Add to the stack
var.awc.s <- stack(var.awc.s, total_var_w20.100_200, total_var_w42.100_200)
...
beginCluster(2)
var_clay_100_200 <- clusterR(granulo.100_200.s, ff, export = list("var_back_CLAY","rho"),
                             filename = paste0("var_clay_",depth,".tif"), format = "GTiff",
                             na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

plot(var_clay_100_200)
...
beginCluster(2)
var_silt_100_200 <- clusterR(granulo.100_200.s, ff, export = list("var_back_SILT","rho"),
                         filename = paste0("var_silt_",depth,".tif"), format = "GTiff",
                         na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

plot(var_silt_100_200)
...
beginCluster(2)
var_sand_100_200 <- clusterR(granulo.100_200.s, ff, export = list("var_back_SAND","rho"),
                         filename = paste0("var_sand_",depth,".tif"), format = "GTiff",
                         na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

plot(var_sand_100_200)
var_clay_100_200 <- raster(paste0("var_clay_",depth,".tif"))
var_silt_100_200 <- raster(paste0("var_silt_",depth,".tif"))
var_sand_100_200 <- raster(paste0("var_sand_",depth,".tif"))

### Clean the workspace a bit
rm(ru_coarse, difference, ff, ru_mm, tic, tac, rho, rho.awc, argile.alr.cub.100_200, silt.alr.cub.100_200,
   argile.alr.cub.100_200,silt.alr.cub.100_200,argile.alr.ckR.100_200,limon.alr.ckR.100_200,
   argile.alr.ckSTD.100_200 , limon.alr.ckSTD.100_200 , argile.alr.limon.alr.cov.100_200, argile.alr.preds.100_200,        
   limon.alr.preds.100_200, clay ,silt , sand,argile.preds.RK.100_200, limon.preds.RK.100_200, sable.preds.RK.100_200,
   var_w20_input_100_200, var_w42_input_100_200, var_w20_ptf_100_200, var_w42_ptf_100_200, var_awc_100_200, total_var_w42.100_200,
   total_var_w20.100_200,coarse.qrF.mean.100_200, coarse.qrF.sd.100_200, coarse.qrF.05.100_200, coarse.qrF.95.100_200,
   SMFC_100_200, SMPWP_100_200, ru_vol_100_200, ru_mm_100_200, silt.alr.v, argile.alr.v, SMFC_100_200.v, SMPWP_100_200.v,
   ptf, coeff.var.covar,rho, ff)

### Save image
save.image("7.6-AWC_Taylor_100_200.RData")
