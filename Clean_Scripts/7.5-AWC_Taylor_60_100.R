...
granulo.60_100.s <- stack(argile.alr.cub.60_100, silt.alr.cub.60_100,
                        ...
                        argile.preds.RK.60_100,limon.preds.RK.60_100,sable.preds.RK.60_100)
...
awc.60_100.s <- stack(SMFC_60_100, SMPWP_60_100,ru_vol_60_100,ru_mm_60_100,coarse.qrF.mean.60_100,awc_mm_60_100)
...
dir.create(paste0(HomeDir,"Clean_Output/7.5-AWC_Taylor_60_100"))
...
tic <- Sys.time()
argile.alr.v <- getValues(argile.alr.preds.60_100)
silt.alr.v <- getValues(limon.alr.preds.60_100)

### calculate correlation
cor.clay.limon.alr.60_100 <- cor.test(x=argile.alr.v, y=silt.alr.v, na.rm=TRUE, method="pearson")
cor.clay.limon.alr.60_100$estimate
tac <- Sys.time()
tac-tic
rm(tac,tic)
...
t_layer <- 400 ### thickness of layer in mm
...
## my intput raster stack is granulo.60_100.s
names(granulo.60_100.s)
...
#  s[[1]] "argile.alr.cub.60_100" 
#  s[[2]] "silt.alr.cub.60_100" 
#  s[[3]] "argile.alr.ckR.60_100" 
#  s[[4]] "limon.alr.ckR.60_100"  
#  s[[5]] "argile.alr.ckSTD.60_100"
#  s[[6]] "limon.alr.ckSTD.60_100"
#  s[[7]] "argile.alr.preds.60_100"
#  s[[8]] "limon.alr.preds.60_100" 
#  s[[9]] "clay" 
#  s[[10]] "silt" 
#  s[[11]] "sand"
...
rho <- cor.clay.limon.alr.60_100$estimate

beginCluster(2)
var_w20_input_60_100 <- clusterR(granulo.60_100.s, ff, export = list("Uncertainty_input","ptf", "rho"),
                          filename = paste0("varInput_w20_",depth,".tif"), format = "GTiff",
                          na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w20_input_60_100 <- raster(paste0("varInput_w20_",depth,".tif"))
...
beginCluster(2)
var_w42_input_60_100  <- clusterR(granulo.60_100.s, ff, export = list("Uncertainty_input","ptf","rho"),
                          filename = paste0("varInput_w42_",depth,".tif"), format = "GTiff",
                          na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w42_input_60_100 <- raster(paste0("varInput_w42_",depth,".tif"))

par(mfrow=c(3,2))
plot(var_w20_input_60_100)
plot(var_w42_input_60_100)
...
beginCluster(2)
var_w20_ptf_60_100 <- clusterR(granulo.60_100.s, ff, export = list("Uncertainty_PTF","coeff.var.covar"),
                        filename = paste0("varPTF_w20_",depth,".tif"), format = "GTiff",
                        na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w20_ptf_60_100 <- raster(paste0("varPTF_w20_",depth,".tif"))
...
beginCluster(2)
var_w42_ptf_60_100 <- clusterR(granulo.60_100.s, ff, export = list("Uncertainty_PTF","coeff.var.covar"),
                        filename = paste0("varPTF_w42_",depth,".tif"), format = "GTiff",
                        na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

gc()
var_w42_ptf_60_100 <- raster(paste0("varPTF_w42_",depth,".tif"))

plot(var_w20_ptf_60_100)
plot(var_w42_ptf_60_100)
...
var.awc.s <- stack (var_w20_input_60_100, var_w42_input_60_100, var_w20_ptf_60_100, var_w42_ptf_60_100)
...
beginCluster(2)
total_var_w20.60_100 <- clusterR(var.awc.s, calc, args = list(sum_unc_w20),
                                 filename =paste0("Total_var_w20.",depth,".tif"), format = "GTiff",
                                 na.rm=T,inf.rm=T, progress = "text", overwrite = T)
endCluster()

total_var_w20.60_100 <- raster(paste0("Total_var_w20.",depth,".tif"))


beginCluster(2)
total_var_w42.60_100 <- clusterR(var.awc.s, calc, args = list(sum_unc_w42),
                              filename =paste0("Total_var_w42.",depth,".tif"), format = "GTiff",
                              na.rm=T,inf.rm=T, progress = "text", overwrite = T)
endCluster()

total_var_w42.60_100 <- raster(paste0("Total_var_w42.",depth,".tif"))

plot(total_var_w20.60_100)
plot(total_var_w42.60_100)

# Add to the stack
var.awc.s <- stack(var.awc.s, total_var_w20.60_100, total_var_w42.60_100)
...
beginCluster(2)
var_clay_60_100 <- clusterR(granulo.60_100.s, ff, export = list("var_back_CLAY","rho"),
                             filename = paste0("var_clay_",depth,".tif"), format = "GTiff",
                             na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

plot(var_clay_60_100)
...
beginCluster(2)
var_silt_60_100 <- clusterR(granulo.60_100.s, ff, export = list("var_back_SILT","rho"),
                         filename = paste0("var_silt_",depth,".tif"), format = "GTiff",
                         na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

plot(var_silt_60_100)
...
beginCluster(2)
var_sand_60_100 <- clusterR(granulo.60_100.s, ff, export = list("var_back_SAND","rho"),
                         filename = paste0("var_sand_",depth,".tif"), format = "GTiff",
                         na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

plot(var_sand_60_100)
var_clay_60_100 <- raster(paste0("var_clay_",depth,".tif"))
var_silt_60_100 <- raster(paste0("var_silt_",depth,".tif"))
var_sand_60_100 <- raster(paste0("var_sand_",depth,".tif"))

### Clean the workspace a bit
rm(ru_coarse, difference, ff, ru_mm, tic, tac, rho, rho.awc, argile.alr.cub.60_100, silt.alr.cub.60_100,
   argile.alr.cub.60_100,silt.alr.cub.60_100,argile.alr.ckR.60_100,limon.alr.ckR.60_100,
   argile.alr.ckSTD.60_100 , limon.alr.ckSTD.60_100 , argile.alr.limon.alr.cov.60_100, argile.alr.preds.60_100,        
   limon.alr.preds.60_100, clay ,silt , sand,argile.preds.RK.60_100, limon.preds.RK.60_100, sable.preds.RK.60_100,
   var_w20_input_60_100, var_w42_input_60_100, var_w20_ptf_60_100, var_w42_ptf_60_100, var_awc_60_100, total_var_w42.60_100,
   total_var_w20.60_100,coarse.qrF.mean.60_100, coarse.qrF.sd.60_100, coarse.qrF.05.60_100, coarse.qrF.95.60_100,
   SMFC_60_100, SMPWP_60_100, ru_vol_60_100, ru_mm_60_100, silt.alr.v, argile.alr.v, SMFC_60_100.v, SMPWP_60_100.v,
   ptf, coeff.var.covar,rho, ff)

### Save image
save.image("7.5-AWC_Taylor_60_100.RData")
