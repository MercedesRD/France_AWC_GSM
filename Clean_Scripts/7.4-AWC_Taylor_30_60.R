...
granulo.30_60.s <- stack(argile.alr.cub.30_60, silt.alr.cub.30_60,
                        ...
                        argile.preds.RK.30_60,limon.preds.RK.30_60,sable.preds.RK.30_60)
...
awc.30_60.s <- stack(SMFC_30_60, SMPWP_30_60,ru_vol_30_60,ru_mm_30_60,coarse.qrF.mean.30_60,awc_mm_30_60)
...
dir.create(paste0(HomeDir,"Clean_Output/7.4-AWC_Taylor_30_60"))
...
tic <- Sys.time()
argile.alr.v <- getValues(argile.alr.preds.30_60)
silt.alr.v <- getValues(limon.alr.preds.30_60)

### calculate correlation
cor.clay.limon.alr.30_60 <- cor.test(x=argile.alr.v, y=silt.alr.v, na.rm=TRUE, method="pearson")
cor.clay.limon.alr.30_60$estimate
tac <- Sys.time()
tac-tic
rem(tac,tic)
...
t_layer <- 300 ### thickness of layer in mm
...
## my intput raster stack is granulo.30_60.s
names(granulo.30_60.s)
### The predictions for clay-alr (regression-kriging) is layer number 8
### the predictions for silt-alr (regression-kriging) is layer number 9
...
#  s[[1]] "argile.alr.cub.30_60" 
#  s[[2]] "silt.alr.cub.30_60" 
#  s[[3]] "argile.alr.ckR.30_60" 
#  s[[4]] "limon.alr.ckR.30_60"  
#  s[[5]] "argile.alr.ckSTD.30_60"
#  s[[6]] "limon.alr.ckSTD.30_60"
#  s[[7]] "argile.alr.preds.30_60"
#  s[[8]] "limon.alr.preds.30_60" 
#  s[[9]] "clay" 
#  s[[10]] "silt" 
#  s[[11]] "sand"
...
rho <- cor.clay.limon.alr.30_60$estimate

beginCluster(4)
var_w20_input_30_60 <- clusterR(granulo.30_60.s, ff, export = list("Uncertainty_input","ptf", "rho"),
                          filename = paste0("varInput_w20_",depth,".tif"), format = "GTiff",
                          na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w20_input_30_60 <- raster(paste0("varInput_w20_",depth,".tif"))
...
beginCluster(4)
var_w42_input_30_60  <- clusterR(granulo.30_60.s, ff, export = list("Uncertainty_input","ptf","rho"),
                          filename = paste0("varInput_w42_",depth,".tif"), format = "GTiff",
                          na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w42_input_30_60 <- raster(paste0("varInput_w42_",depth,".tif"))

par(mfrow=c(3,2))
plot(var_w20_input_30_60)
plot(var_w42_input_30_60)
...
beginCluster(4)
var_w20_ptf_30_60 <- clusterR(granulo.30_60.s, ff, export = list("Uncertainty_PTF","coeff.var.covar"),
                        filename = paste0("varPTF_w20_",depth,".tif"), format = "GTiff",
                        na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

var_w20_ptf_30_60 <- raster(paste0("varPTF_w20_",depth,".tif"))
...
beginCluster(4)
var_w42_ptf_30_60 <- clusterR(granulo.30_60.s, ff, export = list("Uncertainty_PTF","coeff.var.covar"),
                        filename = paste0("varPTF_w42_",depth,".tif"), format = "GTiff",
                        na.rm=T, inf.rm=T, progress = "text", overwrite = T)
endCluster()

gc()
var_w42_ptf_30_60 <- raster(paste0("varPTF_w42_",depth,".tif"))

plot(var_w20_ptf_30_60)
plot(var_w42_ptf_30_60)
...
var.awc.s <- stack (var_w20_input_30_60, var_w42_input_30_60, var_w20_ptf_30_60, var_w42_ptf_30_60)
...
beginCluster(4)
total_var_w20.30_60 <- clusterR(var.awc.s, calc, args = list(sum_unc_w20),
                                 filename =paste0("Total_var_w20.",depth,".tif"), format = "GTiff",
                                 na.rm=T,inf.rm=T, progress = "text", overwrite = T)
endCluster()

total_var_w20.30_60 <- raster(paste0("Total_var_w20.",depth,".tif"))


beginCluster(4)
total_var_w42.30_60 <- clusterR(var.awc.s, calc, args = list(sum_unc_w42),
                              filename =paste0("Total_var_w42.",depth,".tif"), format = "GTiff",
                              na.rm=T,inf.rm=T, progress = "text", overwrite = T)
endCluster()

total_var_w42.30_60 <- raster(paste0("Total_var_w42.",depth,".tif"))

plot(total_var_w20.30_60)
plot(total_var_w42.30_60)

# Add to the stack
var.awc.s <- stack(var.awc.s, total_var_w20.30_60, total_var_w42.30_60)
...
beginCluster(4)
var_clay_30_60 <- clusterR(granulo.30_60.s, ff, export = list("var_back_CLAY","rho"),
...
beginCluster(4)
var_silt_30_60 <- clusterR(granulo.30_60.s, ff, export = list("var_back_SILT","rho"),
...
beginCluster(4)
var_sand_30_60 <- clusterR(granulo.30_60.s, ff, export = list("var_back_SAND","rho"),
...
var_clay_30_60 <- raster(paste0("var_clay_",depth,".tif"))
var_silt_30_60 <- raster(paste0("var_silt_",depth,".tif"))
var_sand_30_60 <- raster(paste0("var_sand_",depth,".tif"))

### Clean the workspace a bit
rm(ru_coarse, difference, ff, ru_mm, tic, tac, rho, rho.awc, argile.alr.cub.30_60, silt.alr.cub.30_60,
   argile.alr.cub.30_60,silt.alr.cub.30_60,argile.alr.ckR.30_60,limon.alr.ckR.30_60,
   argile.alr.ckSTD.30_60 , limon.alr.ckSTD.30_60 , argile.alr.limon.alr.cov.30_60, argile.alr.preds.30_60,        
   limon.alr.preds.30_60, clay ,silt , sand,argile.preds.RK.30_60, limon.preds.RK.30_60, sable.preds.RK.30_60,
   var_w20_input_30_60, var_w42_input_30_60, var_w20_ptf_30_60, var_w42_ptf_30_60, var_awc_30_60, total_var_w42.30_60,
   total_var_w20.30_60,coarse.qrF.mean.30_60, coarse.qrF.sd.30_60, coarse.qrF.05.30_60, coarse.qrF.95.30_60,
   SMFC_30_60, SMPWP_30_60, ru_vol_30_60, ru_mm_30_60, silt.alr.v, argile.alr.v, SMFC_30_60.v, SMPWP_30_60.v,
   ptf, coeff.var.covar,rho, ff)

### Save image
save.image("7.4-AWC_Taylor_30_60.RData")
